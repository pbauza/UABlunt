...
def command_start(m):
    ...
    if cid not in knownUsers or regDone[cid] == False and debugBool[cid] == False:
        set_reg_bools(cid)
        ...
        time.sleep(1)
        bot.send_message(cid, "Necesitamos que contestes unas cuantas preguntas personales para ayudarnos a mejorar.",
                         reply_markup=menu)
    else:
        objeto1[cid] = 0
        objeto2[cid] = 0
        bot.send_message(cid, "¡Bienvenido de nuevo a UABlunt, " + str(m.chat.first_name) + "!", reply_markup=mid_menu)
    ...
    if (gameON[cid] == True):
        ...
        if sit29bool[cid] == False:
            if data[cid] == '29A':
                ...
                time.sleep(2)
                bot.send_message(cid, 'Exhalando el humo, sales de allí como el puto amo que eres.')
                time.sleep(2)
                bot.send_message(cid, '--FIN--', reply_markup=mid_menu)
